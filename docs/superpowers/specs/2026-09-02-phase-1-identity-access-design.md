...
Допустимые переходы: `pending_email_verification` → `active` после подтверждения email; `active` → `blocked`; `blocked` → `active` по отдельному решению администратора безопасности. Блокировка хранит причину, участника, время и ссылку на аудит.
...
2. Администратор безопасности начинает отдельную процедуру только после ручной повторной проверки.
3. Решение содержит проверяемое основание; в первой локальной фазе процедура моделируется и тестируется, но не использует реальные документы.
4. Все сеансы и старые recovery codes отзываются.
5. Пользователь получает `OneTimeToken` для одноразовой ограниченной повторной настройки.
6. Аудит связывает администратора, пользователя, основание и результат.
